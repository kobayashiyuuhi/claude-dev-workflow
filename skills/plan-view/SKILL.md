...
name: plan-view
description: >
  PlanModeのプラン内容をHTML化してブラウザで表示するスキル。ExitPlanModeを呼び出す直前に自動実行される。
...
```bash
mkdir -p .claude/plans   # Mac/Linux
# Windows: New-Item -ItemType Directory -Path .claude\plans -Force
```

### ステップ2: ファイル名の決定

```
.claude/plans/plan-YYYY-MM-DD-HHmm.html
```

例: `.claude/plans/plan-2026-06-03-1430.html`

タイムスタンプ付きで保存する（既存ファイルを上書きしない）。
...
# Windows
start .claude/plans/plan-YYYY-MM-DD-HHmm.html

# Mac
open .claude/plans/plan-YYYY-MM-DD-HHmm.html

# Linux
xdg-open .claude/plans/plan-YYYY-MM-DD-HHmm.html
```
...
```
プラン HTML 出力完了:
  .claude/plans/plan-2026-06-03-1430.html
...
- `.claude/plans/` 配下のHTMLはgit管理対象（設計経緯として保存）
- タイムスタンプ付きファイル名のため既存ファイルを上書きしない
- `.claude/plans/` を `.gitignore` に追加しないこと
