...
| スキル | 説明 |
|--------|------|
| `/dev-setup` | プロジェクト初期セットアップ（フォルダ構成・ドキュメント・git・GitHub） |
...
3. ドキュメントテンプレート配置 (案件書・要件定義書・基本設計書・詳細設計書・設計図など)
4. `.gitignore` 生成
5. `git init` → 初回コミット
6. `README.md` 自動生成
...
セットアップ後は **ウォーターフォール** で進める（ユーザーはお客様兼サポーター）:

```
案件書 → 要件定義書 → 基本設計書 → 詳細設計書 → 開発 → テスト
```

`/dev-setup` は空の器（フォルダ・テンプレ・git・フック）を用意するだけ。中身は上記フェーズで埋める。
最初のフェーズ「案件書」（`document/proposal.md`）は Claude がユーザーへ対話ヒアリングして埋め、
各フェーズ完了時にユーザー承認を得て次へ進む。
